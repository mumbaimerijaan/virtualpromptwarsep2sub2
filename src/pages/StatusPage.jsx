    ...
    <main id="main-content" className="flex-1 flex flex-col bg-[#F9FAFB] pb-32 relative overflow-x-hidden">
      ...
      <div className="px-5 pt-8 pb-6 relative overflow-hidden">
        <div className="w-[60%] relative z-10">
          <h1 className="text-[32px] font-extrabold text-[#1A237E] leading-[1.1] tracking-tight whitespace-pre-line">
            {t.hero.title}
          </h1>
        ...
        <div className="absolute top-0 right-0 bottom-0 w-[45%] pointer-events-none flex items-center justify-end z-0 pr-2">
          <img src={statusHeroImg} alt="Status Illustration" className="w-full h-auto max-h-[140px] object-contain object-right" />
        </div>
      </div>
      ...
      <div className="px-5 flex flex-col gap-6 relative z-10">
        ...
        <div className="flex flex-col gap-4">
          <h2 className="font-bold text-[17px] text-slate-800 tracking-tight">{t.sections.trackingOptions.title}</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="bg-[#F0FDF4] border border-emerald-100 p-5 rounded-[24px] flex flex-col shadow-sm">
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  <h3 className="font-bold text-emerald-900 text-[15px]">{t.sections.trackingOptions.byReference.title}</h3>
                  <p className="text-[12px] text-emerald-700/70 font-medium">{t.sections.trackingOptions.byReference.subtitle}</p>
                </div>
              </div>
              ...
              >
                {t.sections.trackingOptions.byReference.button} <ChevronRight size={18} />
              </a>
              ...
              <div className="mt-4 flex items-center justify-center gap-4 text-[11px] font-bold text-emerald-700/60 uppercase tracking-wider">
                <span className="flex items-center gap-1"><ShieldCheck size={12} /> {t.sections.trackingOptions.badges[0]}</span>
                <span>•</span>
                <span>{t.sections.trackingOptions.badges[1]}</span>
                <span>•</span>
                <span>{t.sections.trackingOptions.badges[2]}</span>
              </div>
            </div>
            ...
            <div className="bg-[#F5F3FF] border border-purple-100 p-5 rounded-[24px] flex flex-col shadow-sm">
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  <h3 className="font-bold text-purple-900 text-[15px]">{t.sections.trackingOptions.byMobile.title}</h3>
                  <p className="text-[12px] text-purple-700/70 font-medium">{t.sections.trackingOptions.byMobile.subtitle}</p>
                </div>
              </div>
              ...
              >
                {t.sections.trackingOptions.byMobile.button} <ChevronRight size={18} />
              </a>
        ...
        <div className="bg-white p-6 rounded-[24px] border border-slate-100 shadow-sm">
          <h3 className="font-bold text-[16px] text-slate-800 mb-8">{t.sections.tracker.title}</h3>
          ...
          <div className="relative px-2">
            ...
            <div className="relative z-10 flex justify-between items-start">
              {t.sections.tracker.steps.map((step, idx) => {
                const icons = [FileText, ShieldCheck, CheckCircle2, XCircle];
                const colors = ['bg-amber-400', 'bg-blue-500', 'bg-emerald-500', 'bg-red-500'];
                const Icon = icons[idx] || FileText;
                return (
                  <div key={idx} className="flex flex-col items-center text-center w-1/4">
                    <div className={`w-12 h-12 rounded-full ${colors[idx]} text-white flex items-center justify-center mb-3 shadow-md border-4 border-white`}>
                      <Icon size={20} />
                    </div>
                    <h4 className="text-[11px] font-bold text-slate-800 leading-tight mb-1">{step.label}</h4>
                    <p className="text-[9px] text-slate-400 font-medium leading-tight max-w-[70px]">{step.description}</p>
                  </div>
                );
              })}
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-3">
              {t.sections.quickHelp.items.map((item, i) => {
                const icons = [MessageCircle, Clock, AlertCircle];
                const colors = ["text-amber-500", "text-blue-500", "text-red-500"];
                const bgs = ["bg-amber-50", "bg-blue-50", "bg-red-50"];
                ...
                return (
                  <button key={i} className="flex items-center justify-between p-4 bg-white rounded-2xl border border-slate-100 shadow-sm hover:bg-slate-50 transition-colors group">
                    <div className="flex items-center gap-4">
                      <div className={`w-10 h-10 rounded-full ${bgs[i]} ${colors[i]} flex items-center justify-center`}>
                        <Icon size={20} />
                      </div>
            ...
            >
              <ExternalLink size={18} /> {t.sections.quickHelp.updateButton}
            </a>

            <button className="w-full py-2 text-[13px] font-bold text-indigo-600 hover:underline flex items-center justify-center gap-1">
              {t.sections.quickHelp.viewAll} <ChevronRight size={14} />
            </button>
          </div>

          {/* What Status Means */}
          <div className="flex flex-col gap-4">
            <h3 className="font-bold text-[17px] text-slate-800 tracking-tight">{t.sections.statusMeaning.title}</h3>
            
            <div className="bg-white p-6 rounded-[24px] border border-slate-100 shadow-sm flex flex-col gap-5">
              {t.sections.statusMeaning.items.map((status, idx) => {
                const colors = ['text-amber-500', 'text-blue-500', 'text-emerald-500', 'text-red-500'];
                return (
                  <div key={idx} className="flex items-start gap-3">
                    <div className={`mt-1 w-2.5 h-2.5 rounded-full ${colors[idx]} bg-current`}></div>
                    <div>
                      <h4 className="text-[14px] font-bold text-slate-800 leading-tight">{status.label}</h4>
                      <p className="text-[12px] text-slate-500 font-medium mt-0.5">{status.description}</p>
                    </div>
                  </div>
                );
              })}
            ...
            <div className="bg-[#FFFBEB] border border-amber-100 p-5 rounded-[24px] flex flex-col gap-4 shadow-sm">
              <div className="flex items-center gap-3 mb-1">
                ...
                <h3 className="font-bold text-amber-900 text-[15px]">{t.sections.nextSteps.title}</h3>
              </div>
              
              <div className="flex flex-col gap-3">
                {t.sections.nextSteps.scenarios.map((sc, i) => (
                  <div key={i} className="p-3 bg-white/60 rounded-xl">
                    <p className="text-[12px] font-bold text-slate-800">{sc.title}</p>
                    <p className="text-[11px] text-slate-600 mt-1">{sc.description}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-white p-4 rounded-[20px] border border-slate-100 shadow-sm flex items-center gap-4 relative overflow-hidden">
               ...
               <div>
                  <h4 className="text-[13px] font-bold text-slate-800">{t.sections.timeTaken.title}</h4>
                  <p className="text-[11px] text-slate-500 font-medium mt-0.5 leading-snug whitespace-pre-line">
                    {t.sections.timeTaken.desc}
                  </p>
               </div>
          ...
          >
            <div className="flex flex-col items-start">
              <span className="text-[15px] flex items-center gap-1.5 leading-tight">
                <ExternalLink size={16} /> {t.cta.title}
              </span>
              <span className="text-[10px] text-indigo-200 font-medium mt-0.5">{t.cta.subtitle}</span>
            </div>
